...
# Deterministic hero asset build: approved-reference clean plates + portrait edge cleanup.
...
for box in [(35,180,580,360),(30,455,440,740),(1070,175,1560,545),(1070,560,1410,745),(30,845,340,930)]:
...
mask_portrait[y0:y0+ph, x0:x0+pw] = np.maximum(mask_portrait[y0:y0+ph, x0:x0+pw], portrait_alpha[:ph,:pw])
...
floor_y, left_end, left_reveal_end, niche_end, right_reveal_end = 786, 593, 628, 1046, 1082


def bilinear_region(width, height, c00, c10, c01, c11):
    xs = np.linspace(0, 1, width)[None, :, None]
    ys = np.linspace(0, 1, height)[:, None, None]
    c00, c10, c01, c11 = [np.array(c)[None,None,:] for c in (c00,c10,c01,c11)]
...
c00,c10,c01,c11 = mean_box(80,30,180,110), mean_box(500,30,570,110), mean_box(80,650,180,740), mean_box(500,650,570,740)
left = bilinear_region(left_end, floor_y, c00,c10,c01,c11)
xx = np.arange(left_end)
left -= (18*np.exp(-((left_end-1-xx)/70)**2))[None,:,None]
clean[:floor_y,:left_end] = np.clip(left,0,255).astype(np.uint8)
...
clean[:floor_y,left_reveal_end:niche_end] = (0.88*niche + 0.12*cv2.GaussianBlur(niche,(0,0),sigmaX=2,sigmaY=2)).astype(np.uint8)
clean[:floor_y,niche_end:right_reveal_end] = stage[:floor_y,niche_end:right_reveal_end]
c00,c10,c01,c11 = mean_box(1120,30,1220,110), mean_box(1510,30,1610,110), mean_box(1120,650,1220,740), mean_box(1510,650,1610,740)
right = bilinear_region(w-right_reveal_end, floor_y, c00,c10,c01,c11)
xx = np.arange(w-right_reveal_end)
right -= (14*np.exp(-(xx/72)**2))[None,:,None]
clean[:floor_y,right_reveal_end:] = np.clip(right,0,255).astype(np.uint8)

c00,c10,c01,c11 = mean_box(350,800,500,840), mean_box(1300,800,1500,840), mean_box(350,845,500,872), mean_box(1300,845,1500,872)
floor = bilinear_region(w,h-floor_y,c00,c10,c01,c11)
xx = np.arange(w)
floor -= (10*np.exp(-((xx-left_end)/48)**2)+7*np.exp(-((xx-niche_end)/50)**2))[None,:,None]
yy = np.arange(h-floor_y)[:,None]
center = np.exp(-((xx-835)/220)**2)[None,:]*np.exp(-((yy-10)/75)**2)
floor -= center[:,:,None]*7
clean[floor_y:] = np.clip(floor,0,255).astype(np.uint8)
orig_band = stage[floor_y-8:floor_y+18].astype(np.float32)
curr_band = clean[floor_y-8:floor_y+18].astype(np.float32)
clean[floor_y-8:floor_y+18] = np.clip(curr_band*.68 + cv2.GaussianBlur(orig_band,(0,0),sigmaX=18,sigmaY=6)*.32,0,255).astype(np.uint8)

stage_clean = Image.fromarray(clean,"RGB")
stage_clean.save(OUT/"hero-architecture-reference-clean.png", optimize=True)
stage_clean.resize((2400,1350),Image.Resampling.LANCZOS).save(OUT/"hero-architecture-desktop.webp","WEBP",quality=92,method=6)
...
def make_plate(width,height,segment_widths,floor_h,filename):
    result=Image.new("RGB",(width,height)); top_h=height-floor_h; x=0
    for (sx1,sx2),target_w in zip(segments,segment_widths):
        ...
        result.paste(crop,(x,0)); x+=target_w
...
rgba=np.array(portrait); rgb=rgba[:,:,:3].copy(); alpha=rgba[:,:,3].copy(); solid=alpha>=248
_,inds=ndimage.distance_transform_edt(~solid,return_indices=True)
nearest_rgb=rgb[inds[0],inds[1]]
partial=(alpha>0)&(alpha<255); lum=rgb.mean(axis=2)
contam=partial&(((alpha<220)&(lum>185))|((alpha<128)&(lum>165)))
weights=np.zeros_like(alpha,dtype=np.float32)
weights[contam]=np.clip((220-alpha[contam])/160,.25,1.0); weights[contam&(lum>235)]=1.0
...
(OUT/"asset-report.txt").write_text(
    f"reference={REFERENCE.relative_to(ROOT)}\nportrait_size={portrait.width}x{portrait.height}\nportrait_mode={portrait.mode}\nalpha_extrema={int(alpha.min())},{int(alpha.max())}\nalpha_bbox={Image.fromarray(alpha).getbbox()}\n",
    encoding="utf-8",
)
print("Hero assets generated")
